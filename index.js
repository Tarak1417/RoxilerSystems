...
app.get('/transactions/stats', async (request, response) => {
  ...
  if (!selectedMonth) {
      response.status(400).json({ error: "Month parameter is required" });
      return;
  }
...
app.get('/combined-data', async (req, res) => {
  try {
      ...
      const apiUrl1 = `http://localhost:${port}/transactions`;
      const apiUrl2 = `http://localhost:${port}/transactions/stats?month=01`;
      const apiUrl3 = `http://localhost:${port}/bar-chart?month=02`;
      const apiUrl4 = `http://localhost:${port}/pie-chart?month=03`;
